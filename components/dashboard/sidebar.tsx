...
export function DashboardSidebar({ collapsed, onToggle }: {
  ...
  // Fetch business name from businesses table (what was saved during onboarding)
  useEffect(() => {
    if (!user) return
    const fetch = async () => {
  ...
  return (
    <>
      <aside className={cn(
        ...
        <div className="flex items-center justify-between px-4 py-5">
          {!collapsed && (
            <EcoTrackLogo size="sm" className="[&_span]:text-sidebar-foreground" />
          )}
        ...
        <div className="border-t border-sidebar-border px-3 py-3">
          {collapsed ? (
            <div
              className="flex h-9 w-9 items-center justify-center rounded-full bg-primary text-xs font-bold text-primary-foreground mx-auto cursor-default"
              title={`${displayName}${businessName ? ` · ${businessName}` : ''}`}>
              {initials}
            </div>
          ) : (
            <div className="flex items-center gap-3">
