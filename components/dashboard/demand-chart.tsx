...
import type { DemandChartPoint } from "@/lib/hooks/use-dashboard"
...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2 shadow-lg">
      ...
      {payload.map((p: any) => (
        <p key={p.name} className="text-xs text-muted-foreground">
          {p.name === "predicted" ? "Predicted" : "Actual"}:{" "}
...
export function DemandChart({ data }: { data: DemandChartPoint[] }) {
  if (!data.length) {
    return (
      <div className="h-[280px] w-full flex items-center justify-center">
        <p className="text-sm text-muted-foreground">No sales data yet. Record sales to see demand.</p>
      </div>
    )
  }
    ...
    <div className="h-[280px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }} barGap={2}>
          ...
          <Bar dataKey="predicted" fill="hsl(152, 55%, 45%)" radius={[4, 4, 0, 0]} maxBarSize={28} />
          <Bar dataKey="actual"    fill="hsl(38, 92%, 50%)"  radius={[4, 4, 0, 0]} maxBarSize={28} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  )
